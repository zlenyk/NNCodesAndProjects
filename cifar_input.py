...
def get_cifar_test_data(filename):
    dict = unpickle(filename)
    return (_convert_images(dict[b'data']), format_labels(dict[b'labels']))
...
def import_cifar():
    ...
    test_set = get_cifar_test_data(os.path.join(data_dir,'test_batch'))
    return (train_images, train_labels), test_set

def import_cifar_gan():
    test_set = get_cifar_data(os.path.join(data_dir,'test_batch'))
    train_images = np.load(os.path.join(data_dir,'gan_images.npy'))
    train_images = np.array(train_images, dtype=float) / 255.0
    train_labels = np.load(os.path.join(data_dir,'gan_labels.npy'))
